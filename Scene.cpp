...
void Scene::init()
{
	...
	//////Enemy
	//GameObject *enemigo = GameObject::make_Object();
...
void Scene::update(int deltaTime)
{
	...
	for (int i = 0; i < objects.size(); ++i) {
		...
		if (objects[i]->getType() == MainPlayer) {
			...
			if ((currentTime / 100) - 30 >= objects[i]->getPosition().x) {
				objects[i]->setPosition(glm::ivec2((currentTime / 100) - 30, objects[i]->getPosition().y));
			}
		}
...
void Scene::render()
{
	...
	map->render();
	texQuad->render(tex);
	float velocity = currentTime / 125;
